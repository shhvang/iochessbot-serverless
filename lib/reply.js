...
export function richBoard(game, premium = true) {
  ...
  const moves = game.moves?.length ? `<p><b>Moves</b><br>${Array.from({ length: Math.ceil(game.moves.length / 2) }, (_, index) => `${index + 1}. ${game.moves[index * 2]}${game.moves[index * 2 + 1] ? ` ${game.moves[index * 2 + 1]}` : ''}`).join('<br>')}</p>` : '';

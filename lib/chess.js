...
export function moveNotation(fen, move) {
  const state = read(fen), piece = state.board[rank(move.from) * 8 + file(move.from)];
  if (move.castle) return move.to[0] === 'g' ? 'O-O' : 'O-O-O';
  const target = state.board[rank(move.to) * 8 + file(move.to)] || (move.enPassant ? 'p' : null);
  const capture = target ? 'x' : '';
  const name = piece.toUpperCase() === 'P' ? (capture ? move.from[0] : '') : piece.toUpperCase();
  const promotion = move.promotion ? `=${move.promotion.toUpperCase()}` : '';
  const after = playMove(fen, move), outcome = after && status(after);
  return `${name}${capture}${move.to}${promotion}${outcome === 'checkmate' ? '#' : outcome === 'check' ? '+' : ''}`;
}
